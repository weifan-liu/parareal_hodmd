...
#This code calculates the example of a single rod-like swimmer with epsilon = 5*sigma for three iterations using the Parareal-HODMD.
...
def Krod_Swave_Wall_FE2(dt,maxt,GridParam,RodsParam,SwaveParam,CurrentTime,ep,ViscosityNu):
    ...
    for i in range(1, maxt + 1):
        ...
        for j in range(NumRods):
            ...
            locX = X[:, StartPt:EndPt]
            locD1 = D1[:, StartPt:EndPt]
            locD2 = D2[:, StartPt:EndPt]
            locD3 = D3[:, StartPt:EndPt]
        ...
        U, W = VelocitiesRegSto7halfRodWall(mf, mn, ep, X, X, np.ones(NumPts), 1)
        U = U / ViscosityNu
        W = W / ViscosityNu
        ...
        for k in range(NumPts):
            ...
            D1[:, k] = np.cos(theta) * D1[:, k] + (1 - np.cos(theta)) * roax * (np.dot(roax_t, D1[:, k])) + np.sin(theta) * np.cross(roax, D1[:, k])
            D2[:, k] = np.cos(theta) * D2[:, k] + (1 - np.cos(theta)) * roax * (np.dot(roax_t, D2[:, k])) + np.sin(theta) * np.cross(roax, D2[:, k])
            D3[:, k] = np.cos(theta) * D3[:, k] + (1 - np.cos(theta)) * roax * (np.dot(roax_t, D3[:, k])) + np.sin(theta) * np.cross(roax, D3[:, k])
        ...
        for j in range(1,NumRods+1):
            StartPt = (j - 1) * M
            EndPt = j * M
            RodLengthVec[i-1, j-1] = np.sum(np.sqrt(np.sum((np.diff(X[:, StartPt:EndPt], 1, 1))**2, axis=0)), axis=0)


    return X, D1, D2, D3, RodLengthVec
...
def Krod_Swave_Wall_RK2_2(dt,maxt,GridParam,RodsParam,SwaveParam,CurrentTime,ep,ViscosityNu):
    ...
    for i in range(1, maxt + 1):
        ...
        U_FE, W_FE = VelocitiesRegSto7halfRodWall(mf, mn, ep, X, X, np.ones(NumPts), 1)
        ...
        for k in range(NumPts):
            ...
            D1_FE[:, k] = np.cos(theta) * D1[:, k] + (1 - np.cos(theta)) * roax * (np.matmul(roax_t, D1[:, k])) + np.sin(theta) * np.cross(roax, D1[:, k])
            D2_FE[:, k] = np.cos(theta) * D2[:, k] + (1 - np.cos(theta)) * roax * (np.matmul(roax_t, D2[:, k])) + np.sin(theta) * np.cross(roax, D2[:, k])
            D3_FE[:, k] = np.cos(theta) * D3[:, k] + (1 - np.cos(theta)) * roax * (np.matmul(roax_t, D3[:, k])) + np.sin(theta) * np.cross(roax, D3[:, k])
        ...
        for k in range(NumPts):
            ...
            D1[:, k] = np.cos(theta) * D1[:, k] + (1 - np.cos(theta)) * roax * (np.matmul(roax_t, D1[:, k])) + np.sin(theta)*np.cross(roax, D1[:, k])
            D2[:, k] = np.cos(theta) * D2[:, k] + (1 - np.cos(theta)) * roax * (np.matmul(roax_t, D2[:, k])) + np.sin(theta)*np.cross(roax, D2[:, k])
            D3[:, k] = np.cos(theta) * D3[:, k] + (1 - np.cos(theta)) * roax * (np.matmul(roax_t, D3[:, k])) + np.sin(theta)*np.cross(roax, D3[:, k])
...
if rank == 0:
    ...
    # This is the array for storing the interpolated coarse-grid solution
...
if rank == 0:
    ...
    # send ic data to rank 2 for computing the G0 in the first serial correction
...
for ii in range(NumParSweeps):
    ...
    for jj in range(ii + 1, NumTimeInterval):
        ...
        if rank == 1:
            ...
            # X_g2, D1_g2, D2_g2, D3_g2, RodLengthVec = Krod_Swave_Wall_FE2(dT, nt, GridParam1, RodsParam, SwaveParam, jj * LengthTimeInterval, ep, ViscosityNu)
            # Assume rr = 1 for now
