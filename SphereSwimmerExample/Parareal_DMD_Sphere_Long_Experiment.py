...
#The motion of the sphere is solved for time interval [0,100]. Parareal-HODMD is applied for one iteration.
...
# Set the value of Kt, the number of intervals calculated by G1 in the initial serial sweep
rtest = .5
...
# Load data for the initial profile of points on the spherical surface
...
if rank == 0:
    ...
    x_true = np.transpose(np.loadtxt("Sphere_Serial_T1_Long_Final.txt"))
...
if rank == 1:
    ...
    for ll in range(ntest):
        X_snapshots_coarse = Sphere_FE2_Snapshots(dT2, CoarseNumTimeStepsPerInterval2, x_sol_c[:,:, ll], rels, ed, K, EP, mu, r_shear, v, A0, rr0)    
    ...
    for ll in range(ntest, NumTimeInterval):
        X_snapshots_coarse_ni = Sphere_FE2_Snapshots(dT2, CoarseNumTimeStepsPerInterval2, x_sol_c[:,:, ll], rels, ed, K, EP, mu, r_shear, v, A0, rr0)    
...
for ii in range(NumParSweeps):
    ...
    for jj in range(ii + 1, NumTimeInterval):
        ...
        if rank == 3 and ii == 0:
            X_snapshots_old_ct0 = Sphere_FE2_Snapshots(dT2, CoarseNumTimeStepsPerInterval2, x_ic_vec[:, :, jj], rels, ed, K, EP, mu, r_shear, v, A0, rr0)
        ...
        if rank == 1:
            ...
            X_snapshots_g2_c = Sphere_FE2_Snapshots(dT2, CoarseNumTimeStepsPerInterval2, ic_x, rels, ed, K, EP, mu, r_shear, v, A0, rr0)
